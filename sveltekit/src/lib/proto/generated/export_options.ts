...
class ExportOptions$Type extends MessageType<ExportOptions> {
    ...
    create(value?: PartialMessage<ExportOptions>): ExportOptions {
        ...
        if (value !== undefined)
            reflectionMergePartial<ExportOptions>(this, message, value);
        return message;
    }
    internalBinaryRead(reader: IBinaryReader, length: number, options: BinaryReadOptions, target?: ExportOptions): ExportOptions {
        let message = target ?? this.create(), end = reader.pos + length;
        while (reader.pos < end) {
            let [fieldNo, wireType] = reader.tag();
            switch (fieldNo) {
                case /* GlobalSettings global_settings */ 1:
                    message.globalSettings = GlobalSettings.internalBinaryRead(reader, reader.uint32(), options, message.globalSettings);
                    break;
                case /* CardInfo card_info */ 2:
                    message.cardInfo = CardInfo.internalBinaryRead(reader, reader.uint32(), options, message.cardInfo);
                    break;
                case /* CardOptions card_options */ 3:
                    message.cardOptions = CardOptions.internalBinaryRead(reader, reader.uint32(), options, message.cardOptions);
                    break;
                default:
    ...
    internalBinaryWrite(message: ExportOptions, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        /* GlobalSettings global_settings = 1; */
        if (message.globalSettings)
            GlobalSettings.internalBinaryWrite(message.globalSettings, writer.tag(1, WireType.LengthDelimited).fork(), options).join();
        /* CardInfo card_info = 2; */
        if (message.cardInfo)
            CardInfo.internalBinaryWrite(message.cardInfo, writer.tag(2, WireType.LengthDelimited).fork(), options).join();
        /* CardOptions card_options = 3; */
        if (message.cardOptions)
            CardOptions.internalBinaryWrite(message.cardOptions, writer.tag(3, WireType.LengthDelimited).fork(), options).join();
